...
验收比想象中的容易（如果是电机就被疯狂拷打）；注意，TCP通信由于供电等原因，经常容易抽风！我验收前就一直抽风，不过展示就没问题了。可以预先录视频证明原本是可行的（还要注意IP等改变的问题）
...
后面进行了一些微调，故有一些地方可能对不上。

由于小组成员过于不靠谱，所以可以说**本项目的温度检测、LCD显示部分实现均依托**。
